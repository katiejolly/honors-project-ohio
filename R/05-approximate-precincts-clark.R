...
clark_geo <- read_csv("R/data/geocoded/clark_geo.csv")

clark_blocks <- blocks(state = "OH", county = "clark") %>%
  st_as_sf()
...
clark_nn <- clark_geo %>%
  ...
  join_voters_to_blocks(clark_blocks, "GEOID10") %>%
  find_neighbors() %>%
  lookup_precincts_nn() %>%
  classify_nn()
...
mapview::mapview(clark_dissolved["PRECINCT"], col.regions = sf.colors(90)) # view the map!!


st_write(clark_dissolved, "R/data/clark_valid_approx.shp")
